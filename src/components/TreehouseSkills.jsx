// src/components/TreehouseSkills.jsx
...
export default function TreehouseSkills({ initialSkills = [] }) {
  ...
  useEffect(() => {
    // Only fetch on client side if no initial skills were provided (client-side fallback)
    if (skills.length === 0) {
      setLoading(true);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {skills.length > 0 ? (
          skills.slice(0, 12).map((badge, index) => (
            <div 
              key={index} 
              className="relative flex items-center justify-center p-3 rounded-md shadow-sm transition-transform transform hover:scale-105 group"
            >
              <img 
                src={badge.icon_url} 
                alt={badge.name} 
